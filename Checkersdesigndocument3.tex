...
\usepackage{tabularx, tabu, longtable}
\usepackage{enumitem}
...
\section{Requirements of Checkers}
	...
	\item Assignment 2 Requirements	
		...
		\item Legal Moves and Crowning \CustomLabel{req:2.2}
			...
			\end{enumerate}
		\end{enumerate}
	\item Assignment 3 Requirements
		\begin{enumerate}[label*=\arabic*.]
			\item \CustomLabel{req:3.1}
			Two Player Game mode
			\begin{enumerate}[label*=\arabic*.]
				\item \CustomLabel{req:3.1.1} Two humans must be allowed to play against each other
				\item \CustomLabel{req:3.1.2} GUI menu must allow for this type of interaction
				\item \CustomLabel{req:3.1.3} Must allow for turn based action depending on the player
			\end{enumerate}
			\item \CustomLabel{req:3.2}
			Single Player Against AI
			\begin{enumerate}[label*=\arabic*.]
				\item \CustomLabel{req:3.2.1} GUI menu must allow for selection of Human vs AI mode
				\item \CustomLabel{req:3.2.2} Both AI and Human opponent must only be allowed to make legal moves such as:
				\begin{itemize}
				\item Move a piece to another square
				\item jump the opponent's piece (so that piece is removed from the board)
				\item convert a piece to a “king”
				\item move kings in both directions (forwards and backwards)
				\item If a capture move is possible it must be taken. If more than one such move is possible, the player chooses which one to make.
				\end{itemize}
				\item Allowing the player to resign the game
				\item \CustomLabel{req:3.2.3} Indicating moves graphically or by code (E3-D4 etc), or both. 
			\end{enumerate}
			\item \CustomLabel{req:3.3}
			Game play rules
			\begin{enumerate}[label*=\arabic*.]
				\item \CustomLabel{req:3.3.1} Indicate if the game has been won
				\item \CustomLabel{req:3.3.2} Include any on screen help you think is necessary or helpful.
			\end{enumerate}
		\end{enumerate}
	\end{enumerate}
...
\section{Revision History}
	...
	\subsection{Board}
	\begin{itemize}
	\item Added the ability to see if a jump is available on the board.
    \item The piece that is currently jumping is saved. 
    \item Added the ability to retrieve the number of Pieces on the board.
	\end{itemize}
        ...
        \begin{tabularx}{\linewidth}{@{} >{\bfseries}r Xp{5cm} }
            ...
            Access Programs & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
									...
									setValidMovements (direction : validMoveDirection, col : int, row : int) & Assigns the valid movements for a piece\\
                                     & \\
    								canJump () & Returns true if there is a possible jump.\\
                              \end{tabular}
        \end{tabularx}
        ...
        \begin{longtabu}{ >{\bfseries}r Xp{2cm} }
            ...
            Access Programs & \\
                            ...
                            & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                ...
                                Updates & None \\ 
                                Outputs & None \\
                                Description & Returns true if there is a jump available and false otherwise.\\
                                 & \\   
                            \end{tabular} \\
        \end{longtabu}%
%
    ...
    \subsubsection{Interface}
      ...
      \begin{tabularx}{\linewidth}{@{} >{\bfseries}r Xp{5cm} }
            Access Programs & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                    ...
                                    & \\
                                    \hspace{0pt}setJumpingPiece(newJumpingPiece: Piece) & Set the piece that is currently jumping.  \\
                                    & \\
...
\begin{tabular}{c c}
\hline \hline
Case Number & Description \\ [0.5ex]
\hline
1 & \begin{tabular}{@{}c@{}}If there is a jump available and the jumping piece is set \\ check if that piece can continue jumping and jump with it, else end the turn\end{tabular} 

\\
2 & \begin{tabular}{@{}c@{}}If there is a jump available and there is no piece currently \\ jumping then make the jump and set which piece is jumping\end{tabular} 

\\
3 & \begin{tabular}{@{}c@{}}If there is no jump available and no piece has jumped yet \\ then move normally (not jumping) and change the turn\end{tabular} 
...
\end{table}
        \begin{longtabu}{ >{\bfseries}r Xp{2cm} }
            ...
            Access Programs & \\
                                    & \\
                                & \bf{Update()} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                    Inputs &  None \\
                                    ...
                                    Description & Changes the state based on keyboard press or mouse presses on graphical buttons.\\
                                     & \\
                                    \end{tabular} \\
                                & \bf{Draw()} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                    Inputs & board \\
                                    Updates & pieceList \\
                                    Outputs & None \\
                                    Description & Draws the buttons, board tiles and pieces in proper place on the screen. The piece locations are stored in pieceList. And we just loop through the graphics objects to draw them each frame. \\
                                     & \\
                                     \end{tabular} \\
                                & \bf{takeInput()} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                    Inputs & input \\
                                    Updates & None \\
                                    Outputs & board \\ 
                                    ...
                                    & \\
                                    \end{tabular} \\
                                & \bf{setValidMovements(Board : board)} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                    Inputs & Board \\
                                    Updates & Board \\
                                    Outputs & None \\
                                    ...
                                    & \\
                                    \end{tabular} \\                                
                                & \bf{setValidMovements(Board : board, X : int, Y : int)} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
                                    Inputs & Board, X , Y \\
                                    Updates & Board \\
                                    Outputs & None \\
                                    ...
                                    & \\
                                    \end{tabular} \\
                                & \bf{win(PLAYER : player)} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
          		                    Inputs & PLAYER \\
                                    Updates & Board, PLAYER \\
                                    Outputs & None \\
                                    Description & This function will be called when the win condition is true and will clear and reset the board while calling other functions to signal the finish of the game \\
                                    & \\
                                    \end{tabular} \\                            
                                & \bf{changeTurn()} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
          		                    Inputs & None \\
                                    ...
                                    \end{tabular} \\
                                &\bf{moveTimerTick(source : Object, e : ElapsedEventArgs)} \\ & \begin{tabular}[t]{@{} p{4cm} p{8cm}}
          		                    Inputs & None \\
                                    Updates & CurrentPlayerTurn, Board \\
                                    Outputs & None \\
                                    Description & Initializes the timer component for each turn, and will execute the win condition if the timer has counted down to zero. \\
                                \end{tabular} \\
        \end{longtabu}
...
Evidently, our design makes use of several essential design principles for simplicity and efficiency. Our design makes use of a hierarchical structure to make the system easier to build and test. We made use of abstraction by having the program abstract the whole game, the game abstract the board, the board abstract the pieces, etc. so we could start assigning different parts to the group right away. We also used the idea of information hiding to make things that are likely to change private. This maximized efficiency and allowed us to get our design done very quickly. Our design makes use of the high cohesion and low coupling principles as much as possible to make sure our modules are meaningful when standing alone. We made a variety of decisions that would enhance our design, including the following examples. The setting of valid moves is done incredibly efficiently, as we use integer comparisons in the logic section and assign the valid moves to every piece, which is much faster than if every piece's moves were stored on the piece array. Special types in the design are enumerations so they can be converted quickly. We minimize code duplication by reusing the jump logic for the AI.  Our jump logic, as we can see from the jump table, is both complete and disjoint, to cover all possibilities while keeping things simple. Finally, the Struct used combines all the information needed in one place and is very intuitive. Overall we made conscious decisions in the design to ensure that the principles of software design were followed closely.        
\end{document}